...
const TaxWizard: React.FC<TaxWizardProps> = ({ initialTab, onOpenFullCalculator, onBack }) => {
  ...
  const totalSteps = 4; // 0-indexed: 0=who, 1=income, 2=deductions, 3=result

  // ── Step 0: Who ──
  const StepWho = () => (
    <div>
      <ProgressBar step={0} total={totalSteps} />
      <BigLabel>Who are you calculating tax for?</BigLabel>
      <Sub>Pick one to get started — takes less than 2 minutes.</Sub>
  ...
  const StepIncome = () => (
    <div>
      ...
      {state.taxType === 'personal' ? (
        <>
          <BigLabel>What's your annual gross income?</BigLabel>
          <Sub>Enter the total before any deductions or tax — usually what's on your offer letter or payslip.</Sub>
          <AmountInput
        ...
        <>
          <BigLabel>What's your company's annual turnover?</BigLabel>
          <Sub>Total revenue before any deductions. This determines your company size and tax rate.</Sub>
          <AmountInput
  ...
  const StepDeductions = () => (
    <div>
      ...
      {state.taxType === 'personal' ? (
        <>
          <BigLabel>Which deductions apply to you?</BigLabel>
          <Sub>These reduce your taxable income. Toggle on anything that applies — you can change them later.</Sub>
        ...
        <>
          <BigLabel>A couple more details</BigLabel>
          <Sub>These affect your company's tax rate and available exemptions.</Sub>
          ...
          <AmountInput
            ...
            hint="Used to determine small company status. Must be under ₦250M alongside turnover ≤ ₦100M"
          />
        </>
      )}
      <NavRow
  ...
  const StepResult = () => {
    ...
    if (state.taxType === 'personal') {
      ...
      const assets = parse(state.fixedAssets);
    ...
    return (
      <div>
        ...
        {personalResult && (
          <>
            <BigLabel>Your tax breakdown</BigLabel>
            <Sub>Based on NTA 2025. Effective rate: {personalResult.effectiveRate.toFixed(1)}%</Sub>
        ...
        {companyResult && (
          <>
            <BigLabel>Your company tax breakdown</BigLabel>
            <Sub>
        ...
        <div className="flex gap-3">
          <button
            onClick={() => { setState({ ...initial, taxType: null }); setStep(0); }}
